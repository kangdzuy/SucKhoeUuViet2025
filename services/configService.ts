...
const KEY_PREFIX = 'PAC_CONFIG_';

// Default Product ID
const DEFAULT_PRODUCT = 'uv2025';
...
const DEFAULT_BASE_RATES: RateTable = {
    ...
    A_MEDICAL_LOW_VN: [1.56, 0.96, 1.08, 1.20, 1.56], 
    A_MEDICAL_MID1_VN: [1.22, 0.75, 0.84, 0.94, 1.22], 
    A_MEDICAL_MID2_VN: [1.01, 0.62, 0.70, 0.78, 1.01], 
    A_MEDICAL_HIGH_VN: [0.86, 0.53, 0.60, 0.66, 0.86], 
    ...
    // --- BENEFIT B ---
    B_VN: [0.34, 0.21, 0.24, 0.27, 0.34], 
    B_ASIA: [0.43, 0.27, 0.30, 0.33, 0.43], 
    B_GLOBAL: [0.48, 0.30, 0.33, 0.37, 0.48], 

    // --- BENEFIT C ---
 
    C_VN_BAND1: [3.50, 3.23, 2.15, 2.69, 3.50],
    C_VN_BAND2: [3.24, 2.99, 1.99, 2.49, 3.24],
    C_VN_BAND3: [2.85, 2.63, 1.76, 2.19, 2.85],
    C_VN_BAND4: [2.46, 2.27, 1.51, 1.89, 2.46],
    ...
    E_VN_BAND1: [31.68, 19.50, 29.25, 24.37, 31.68],
    E_VN_BAND2: [26.03, 16.02, 24.03, 20.02, 26.03],
    E_VN_BAND3: [25.49, 15.69, 23.53, 19.61, 25.49],
...
const DEFAULT_MIN_RATES: RateTable = {
    ...
    A1_VN: [0.0750, 0.0462, 0.0519, 0.0577, 0.0750], 
...
export const configService = {
  // Support Multi-product configuration
  getConfig: (productId: string = DEFAULT_PRODUCT): SystemConfig => {
    try {
      ...
      // Fallback for legacy data (migration support)
      if (productId === DEFAULT_PRODUCT) {
          const legacy = localStorage.getItem('PAC_CALC_CONFIG_V5');
          if (legacy) return JSON.parse(legacy) as SystemConfig;
      }
    ...
    // In a real app, you might want different defaults per product ID
    return DEFAULT_CONFIG;
  },

  saveConfig: (config: SystemConfig, productId: string = DEFAULT_PRODUCT) => {
    try {
      const key = `${KEY_PREFIX}${productId}`;
  ...
  resetConfig: (productId: string = DEFAULT_PRODUCT) => {
    const key = `${KEY_PREFIX}${productId}`;
    localStorage.removeItem(key);
    // Also remove legacy if default
    if (productId === DEFAULT_PRODUCT) localStorage.removeItem('PAC_CALC_CONFIG_V5');
